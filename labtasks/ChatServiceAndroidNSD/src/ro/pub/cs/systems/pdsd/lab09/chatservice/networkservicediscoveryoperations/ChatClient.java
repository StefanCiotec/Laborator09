			...
			if (printWriter != null) {
				try {
					...
					while (!Thread.currentThread().isInterrupted()) {
						...
						Log.d(Constants.TAG, "Sending the message "+ str);
			...
			if (bufferedReader != null) {
				try {
					...
					while (!Thread.currentThread().isInterrupted()) {
						...
						if (str != null) {
							Log.d(Constants.TAG, "Received the message "+ str);
							Message message = new Message(str, Constants.MESSAGE_TYPE_RECEIVED);
							conversationHistory.add(message);
							if (context != null) {
							  ChatActivity chatActivity = (ChatActivity)context;
							  FragmentManager fragmentManager = chatActivity.getFragmentManager();
